...
public class Tp3Ej3 {
    ...
    public List<String> devolverCamino (String ciudad1, String ciudad2){
        ...
        if (!mapaCiudades.isEmpty()){
            ...
            Vertex<String> fin = mapaCiudades.search(ciudad2);

            if (vertice!=null && fin!=null){
                boolean [] marca = new boolean[mapaCiudades.getSize()];
                for (int i=0; i<mapaCiudades.getSize(); i++)
                    recorridoDevolverCaminoDFS(0, mapaCiudades, marca, ciudad1, ciudad2);
            }
        }
    ...
    private void recorridoDevolverCaminoDFS(int i, Graph<String> grafo, boolean [] marca, String ciudad1, String ciudad2){
        marca[i] = true;
        Vertex<String> v = mapaCiudades.getVertex(i);

        // System.out.println(v);
        // tengo que procesar el camino act
        boolean encontre = false;

        List<Edge<String>> adyacentes = mapaCiudades.getEdges(v);
        for (Edge<String> e: adyacentes){
            int j = e.getTarget().getPosition();
            if (e.getTarget().getData().equals(ciudad2)){
                encontre = true;
            }
            if (!marca[j]){
                recorridoDevolverCaminoDFS(j, grafo, marca);
            }
            if (encontre)
                break;
        }
